...
const API_NAME_EXIST = API_PROJECT + '/is-name-exist';
...
export class ProjectService {
  ...
  getAllProject(): Observable<ProjectItem[]> {
    return this.http.get<ProjectItem[]>(API_PROJECT).pipe(map((projects) => projects.map((response) => ProjectItem.fetchProjectFromResponse(response))));
  }
  ...
  getParentOptions(id: number): Observable<ProjectItem[]> {
    return this.getApparentableProject(id).pipe(map((projets) => this.generateParentOption(projets, 0)));
  }
  ...
  isNameUnique(name): Observable<boolean> {
    return this.http.get<boolean>(`${API_NAME_EXIST}/${name}`);
  }
